  ...
  return (
    <div>
      ...
      <div className='grid md:grid-cols-2 grid-cols-1 lg:grid-cols-3 gap-6'>
        {
          data.map((item, index) => (
            <div key={index} className='bg-white shadow-md h-96 mx-3 rounded-3xl flex md:flow-row justify-around items-center overflow-hidden sm:flex-grow sm:h-52 sm:w-300'>
              ...
              <div className='flex-1 w-full flex flex-col items-baseline justify-around h-1/2 pl-6 sm:h-full sm:items-baseline sm:w-1/2'>
                ...
                <div className='flex gap-2'>
                  ...
                  <Link to={`/dashboard/course-details`}><button className='bg-secondary font-bold rounded-xl mr-5 text-white px-3 py-1 shadow-md'>
                    View
                  </button></Link>
                </div>
              </div>
