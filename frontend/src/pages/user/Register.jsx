...
//                      Confirm Password
//                   </label>
//                   <input type="password" placeholder='Confirm Password' {...register("confirmPassword", { required: true, validate: (value) => value === password || "Password does not match" })}
//                   className='w-full border-gray-300 border rounded-md py-2 px-4 focus:outline-none focus:ring focus:border-blue-300'
//                   />
//                </div>
//             </div>
            ...
            .then(() => {
              ...
              if (userImp.email && user.displayName) {
                return axios.post('http://localhost:3000/new-user', userImp)
                  ...
                  .catch((err) => {
                    setGeneralError(err.response?.data?.message || 'An error occurred. Please try again.');
                  });
              }
            })
        }
    ...
    <div className='flex justify-center items-center pt-14 bg-gray-100'>
      <div className='bg-white p-8 rounded-lg shadow-md'>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className='text-center'>
            <button type='submit' className='bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-md'>Register</button>
          </div>
          {generalError && <div className='text-red-500 text-sm w-full mt-1'>{generalError}</div>}
        </form>
        <p className='text-center mt-4'>Already have an account? <Link to='/login' className='underline text-blue-500 ml-1'>Login</Link></p>
        <GoogleLogin />
      </div>
    </div>
  )
